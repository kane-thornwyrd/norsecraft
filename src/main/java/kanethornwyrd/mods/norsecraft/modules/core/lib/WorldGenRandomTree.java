...
import kanethornwyrd.mods.norsecraft.modules.core.blocks.ModLog;
...
protected WorldGenRandomTree.WorldGenRandomTreeConfig CONFIG;

public WorldGenRandomTree( boolean notify, Block log, Block leaves ) {
  super(notify);
...
public boolean generate( World worldIn, Random rand, BlockPos position ) {
  ...
  if (k >= 1 && k + i + 1 < 256) {
    ...
    boolean isSoil = state.getBlock().canSustainPlant(state, worldIn, blockpos, net.minecraft.util.EnumFacing.UP, (
      (net.minecraft.block.BlockSapling) Blocks.SAPLING));
    
    if (!(isSoil && position.getY() < worldIn.getHeight() - i - 1)) {
      return false;
    } else if (this.placeTreeOfHeight(worldIn, position, (int) Math.ceil(CONFIG.height))) {
      this.onPlantGrow(worldIn, blockpos, position);
      
      new WorldGenRandomTree.RandomTree(worldIn, rand, position);
      return true;
    } else {
      return false;
    }
...
public WorldGenRandomTree configure( Map<String, Float> confMap ) {
  CONFIG = new WorldGenRandomTree.WorldGenRandomTreeConfig(
...
private class Branch {
  ...
  public Branch( World worldIn, Branch parent, BlockPos pos, Vec3d dir, int girth ) {
    ...
    this.parent = parent;
    this.dir = dir;
    for (int w = 0; w < girth; w++) {
      for (int d = 0; d < girth; d++) {
        for (int h = 0; h < girth; h++) {
          BlockPos newPos = pos.add(w, h, d);
          drawLine(worldIn, newPos, new BlockPos(pos.getX(), pos.getY() * dir.y, pos.getZ()));
        }
      }
    }
  }
...
private class Leaves {
  final protected int LEAVES_RADIUS = 4;
  Vec3d pos;
  World worldIn;
  ...
  public Leaves draw() {
    ...
    if (LEAVES != null) {
      ...
      z2 = z0 + r;
      for (; x1 <= x2; x1++) {
        for (y1 = y0 - r; y1 <= y2; y1++) {
          for (z1 = z0 - r; z1 <= z2; z1++) {
            double check = Math.pow(x1 - x0, 2) + Math.pow(y1 - y0, 2) + Math.pow(z1 - z0, 2);
            if (check + LEAVES_RADIUS * 2 < r2) {
...
private class RandomTree {
  private Set<Leaves> leaves;
  private Set<Branch> branches;
  private List<Branch> root;
  
  public RandomTree( World worldIn, Random rand, BlockPos origin ) {
    ...
    int density = CONFIG.cubage / CONFIG.maxHeight;
    
    for (int i = 0; i < density; i++) {
      
      int xp = xa + rand.nextInt(Math.abs(xz - xa) | 1);
      int yp = ya + rand.nextInt(Math.abs(yz - ya) | 1);
      int zp = za + rand.nextInt(Math.abs(zz - za) | 1);
...
public class WorldGenRandomTreeConfig {
  int girth;
  int minHeight;
  int maxHeight;
  int height;
  ...
  public WorldGenRandomTreeConfig(
    int girth,
    int minHeight,
    int maxHeight,
    int cubage,
