...
public class WorldGenSpaceColonizationTree extends WorldGenAbstractTree {
...
public WorldGenSpaceColonizationTree( boolean notify, Block log, Block leaves ) {
  super(notify);
...
public boolean generate( World worldIn, Random rand, BlockPos position ) {
  ...
  if (k >= 1 && k + i + 1 < 256) {
    ...
    if (!(isSoil && position.getY() < worldIn.getHeight() - i - 1)) {
      ...
      new FractalTree(worldIn, rand, position);
    }
    return true;
  }
...
public WorldGenSpaceColonizationTree configure( Map<String, Float> confMap ) {
...
private class FractalTree {
  ...
  public FractalTree( World worldIn, Random rand, BlockPos origin ) {
    ...
    for (int i = 0; i < density; i++) {
      ...
      int xp = xa + rand.nextInt(Math.abs(xz - xa));
      int yp = ya + rand.nextInt(Math.abs(yz - ya));
      int zp = za + rand.nextInt(Math.abs(zz - za));
